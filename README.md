...
1- monsters variable boundaries: position, energy,confusion turns(maybe)...
2- compareto mechanism (90% already correct)
3-he didnt specify doing a method in interface why did we do it?is it fine? he said add if you want
4-access modifiers of classes are now default so other packages cant see a package so we have to import. is this fine or do we have to change every access modifier from default to public ( I think its fine but everyone has to take care to import the class if it is in a different package to avoid error)
